...
def evaluate_model(nx, ny, nz, cell_len, CHANNEL_N, ca, sp_rate, delta_t, run_n=10, max_frame=120, ini_seed=40, ini_mode='random', ini_pos='random', ea_mode='single', ea_normalize=[85.0 * np.pi / 180.0, 100.0], include_T=False, T_ini=20.0, cooling=False, cr=0.0):
    for run_i in range(run_n):
        rsme_tmp, acc_ca_tmp, acc_tmp, rsme_misori_tmp, com_sp_tmp, ca_sp_tmp, nca_cpu_speed  = CA_routine(nx, ny, nz, cell_len, CHANNEL_N, ca, run_i, sp_rate, delta_t, ini_seed, ini_mode, ini_pos, ea_mode, ea_normalize, include_T, T_ini, cooling, cr)
        rsme_tmp = np.array(rsme_tmp)*100.0
        acc_tmp = np.array(acc_tmp)*100.0
        acc_ca_tmp = np.array(acc_ca_tmp) * 100.0
        com_sp_tmp = np.array(com_sp_tmp)
        ca_sp_tmp = np.array(ca_sp_tmp)
        nca_cpu_speed = np.array(nca_cpu_speed)
    ...
    print("The total computation time for NCA in GPU is " + str(np.sum(com_sp_tmp)) + " s ")
    print("The total computation time for NCA in CPU is " + str(np.sum(nca_cpu_speed)) + " s ")
    print("The total computation time for CA is " + str(np.sum(ca_sp_tmp)) + " s ")
    # save the model rsme and accuracy at different run
    np.savetxt('model_rsme.csv', rsme, delimiter=',')
    np.savetxt('model_acc_ca.csv', acc_ca, delimiter=',')
    ...
    acc_ca_m = np.mean(acc_ca, axis=0)
    acc_ca_std = np.std(acc_ca, axis=0)
    acc_m = np.mean(acc, axis=0)
    acc_std = np.std(acc, axis=0)
    rsme_misori_m = np.mean(rsme_misori, axis=0)
    rsme_misori_std = np.std(rsme_misori, axis=0)
    print("The RSME of misorientation for speed-up CA in last frame is "+str(rsme_m[-1])+ str(u" \u00B1 ") + str(rsme_std[-1]))
    print("The accuracy of prediction for speed-up CA in last frame is " + str(acc_ca_m[-1]) + str(u" \u00B1 ") + str(acc_ca_std[-1]) + " % ")
    print("The accuracy of prediction for NCA in last frame is " + str(acc_m[-1])+str(u" \u00B1 ")+str(acc_std[-1])+" % ")
    print("The RSME of misorientation For NCA in last frame is " + str(rsme_misori_m[-1]) + str(u" \u00B1 ")+ str(rsme_misori_std[-1])+" % " )
...
cooling = False#True
cr=0.0
T_ini = 20.0 #np.repeat(np.linspace(20, 25, nx)[..., None, None], ny, 1)# 22.5#
...
for i in [40,160,400]: #40,  160,  280,  400,  520,
    nx = ny = int(i)
