            ...
            with VideoWriter('ca_speedup.mp4') as vid_ca:
                while t_step >= 0:
                    ...
                    if t_step_ca == 0:
                        ...
                        #np.savetxt('ini_state.csv',[ran_x,ran_y,ran_ea_i[...,0]],delimiter=',')
                        '''
                        ran_x, ran_y, ran_ea_i = np.loadtxt('ini_state.csv', delimiter=',')
                        ran_ea_i = ran_ea_i[...,None]
                        ran_x = ran_x.astype(int)
                        ran_y = ran_y.astype(int)
                        ...
                        for ran_i in range(seed_num):
                            for ran_i2 in range(seed_num):
                                ran_x.append(int(ran_i*40+20))
                                ran_y.append(int(ran_i2 * 40 + 20))
                                ran_ea_i.append(45.0)
                        ran_x = np.array(ran_x)
                        ran_y = np.array(ran_y)
                        ran_ea_i = np.array(ran_ea_i)[...,None]
                        seed_num = (nx // 40)**2
                    ...
                    if t_step_ca == 0:
                        ...
                        nca_cpu_speed.append(end - start)
                        if liq_cell_ca != 0.0:
                        ...
                        filter = mis_ori > 15.0
                        acc_ca.append(1.0 - np.sum(filter) / (nx * ny))
                    ...
                    if (liq_cell_ca == 0.0) or t_step == 1000000:  # if there is no liquid or the time step is too much
                        ...
                        if t_step_ca % sp_rate != 0:
                            np.delete(ca_speed,list(np.array(range(-int(t_step_ca % sp_rate),0))))
                            print("recorded ca step number:",len(ca_speed))
                        if device.type=='cuda':
                            x_train = (x.permute(0, 2, 3, 1)).detach().cpu().numpy()
                        else:
                            x_train = (x.permute(0, 2, 3, 1)).detach().numpy()
                        ...
                        '''
                        print(t_step_ca)
                        return rsme, acc_ca, acc, rsme_misori, NCA_comp_speed, ca_speed, nca_cpu_speed 
