            ...
            with VideoWriter('ca_speedup.mp4') as vid_ca:
                while t_step >= 0:
                    ...
                    if t_step_ca == 0:
                        ...
                        # np.savetxt('ini_state'+str(run_i)+'.csv',[ran_x,ran_y,ran_ea_i],delimiter=',')
                    ...
                    if (liq_cell_ca == 0.0) or t_step == 1000000:  # np.sum((x.permute(0, 2, 3, 1)).detach().numpy()[...,3:4]<0.99)==0  if there is no liquid or the time step is too much
